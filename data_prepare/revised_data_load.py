...
def main(train_ratio = 1.0,):
  ...
  # numpy 配列に変換
  omega_train = np.hstack(train_omega_list) if train_omega_list else np.array([])
  gain_train  = np.hstack(train_gain_list)  if train_gain_list  else np.array([])
  phase_train = np.hstack(train_phase_list) if train_phase_list else np.array([])
  ...
  # テストセットがある場合
  if test_paths:
    omega_test = np.hstack(test_omega_list) if test_omega_list else np.array([])
    gain_test  = np.hstack(test_gain_list)  if test_gain_list  else np.array([])
    phase_test = np.hstack(test_phase_list) if test_phase_list else np.array([])
  ...
  # 確認
  # # 各配列を 1 次元に連結
  combined_omega_list = train_omega_list + test_omega_list
  combined_gain_list  = train_gain_list  + test_gain_list
  combined_phase_list = train_phase_list + test_phase_list

  omega     = np.hstack(combined_omega_list) if combined_omega_list else np.array([])
  sys_gain  = np.hstack(combined_gain_list)  if combined_gain_list  else np.array([])
  sys_phase = np.hstack(combined_phase_list) if combined_phase_list else np.array([])
...
def main_individual(train_ratio = 1.0,):
  dat_dir = Path(__file__).parent
  dat_paths = sorted(dat_dir.glob("*.dat"))
  k = len(dat_paths)
  num_train = int(train_ratio * k)
  ...
  z_train = gain_train * np.exp(1j * phase_train)
  mask_train = np.abs(z_train) < 0.8
  Y_train_1 = z_train.real[mask_train]
  Y_train_2 = z_train.imag[mask_train]
  X_train = omega_train[mask_train]
  ...
  if test_paths:
    ...
    Y_test_1 =z_test.real[mask_test]
    Y_test_2 =z_test.imag[mask_test]
    X_test = omega_test[mask_test]
  else:
    Y_test_1 = np.empty((0,))
    Y_test_2 = np.empty((0,))

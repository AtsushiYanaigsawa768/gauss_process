...
class LaplaceGaussianProcess:
    def __init__(self, scale=1.0, n_restarts_optimizer=25):
        self.scale = scale  # Scale parameter for Laplace distribution
    ...
    def fit(self, X, y):
        ...
        # Iterative robust fitting with Laplace distribution weights
        for i in range(3):  # Perform a few iterations of reweighting
            y_pred = self.gpr.predict(X)
            residuals = y - y_pred
            self.scale = np.mean(np.abs(residuals))  # MLE for Laplace scale parameter
            
            # Compute weights based on Laplace distribution
            weights = self._compute_laplace_weights(residuals)
    ...
    def _compute_laplace_weights(self, residuals):
        """Compute weights based on Laplace distribution density relative to Gaussian"""
        # Get Laplace distribution PDF at residuals
        laplace_pdf = stats.laplace.pdf(residuals, scale=self.scale)
        
        # Get normal PDF at residuals for comparison
        normal_pdf = stats.norm.pdf(residuals, scale=np.std(residuals))
        
        # Compute weights (avoid division by zero)
        weights = np.maximum(laplace_pdf / (normal_pdf + 1e-10), 1e-10)
...
plt.legend()
plt.grid(True)
plt.savefig(f"/root/gauss_process/result/laplase_distribution.png")
plt.close()